...
MoviePilot v2 插件：订阅新增时优先搜索 Telegram 公开频道、观影资源站和聚影 API；匹配的 115 分享直接转存，确认匹配的完整观影磁力通过 CMS 离线到 115，失败时回退到 MoviePilot 默认搜索。

当前本地版本：`v4.5.1`。该版本把观影磁力出口固定为 CMS/115 离线任务，继续复用 MoviePilot 规则与 TMDB/豆瓣身份确认，并修复 CMS 只读检查关闭配置弹窗的问题。
...
- 磁力不进入 115 `share_receive`，而是作为 `TorrentInfo.enclosure` 提交给 MoviePilot `DownloadChain`，沿用订阅下载器、保存路径和后续整理规则写入 115。
