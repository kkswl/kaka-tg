...
当前版本：`v4.7.33`。新增 PanSou 聚合搜索，115 分享候选会受限地只读读取文件名，用真实文件名执行字幕、清晰度、季号和 TMDB/豆瓣确认；有限确认优先用于显式媒体 ID、作品页及正式名/别名明确命中的高可信候选。确认后的实际处理顺序为 TG 115、观影 115、PanSou 115、观影磁力、PanSou 磁力、聚影，手动真实处理也必须绑定订阅并由后端重新确认。
