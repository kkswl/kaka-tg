...
当前版本：`v4.7.53`。手动搜索的“搜索范围”位于搜索框上方，选择后点击搜索生效；下方“结果来源”用于即时筛选已有结果，界面位置有对应契约测试保护。手动搜索 API 统一返回资源类型、网盘类型、画质、中文字幕、原盘、季号和年份等结构化字段，页面优先按结构化字段筛选并兼容旧会话缓存。CMS 配置、状态和操作入口已从用户界面及公开配置输出隐藏；历史 `CmsTaskLedger` 名称与持久化键仅用于 115 磁力幂等、状态轮询和重启恢复，不会调用外部 CMS。
