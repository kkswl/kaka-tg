...
当前版本：`v4.7.39`。TG115 会固定首次通过规则与媒体身份确认的磁力 BTIH 顺序，每次只运行一个候选；明确取消、失败或无资源后自动选择下一个不同 BTIH，默认最多提交 5 个。写请求超时或查询失败进入 `unknown` 并保持 TG115 所有权，按 BTIH 对账确认旧任务不存在前不会提交下一个，也不会释放 MoviePilot。下载中、已下载和待整理状态继续阻断 MoviePilot 原生搜索；候选耗尽或达到上限后才允许最终回退。队列顺序、索引、尝试历史和所有者通过插件数据持久化；电视剧在途期间通过保持订阅 `state=P` 阻断 MoviePilot 补搜，终态后按媒体库真实缺集重新处理。
