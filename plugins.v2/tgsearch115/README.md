...
当前版本：`v4.7.35`。新增 PanSou 聚合搜索，115 分享候选会受限地只读读取文件名，用真实文件名执行字幕、清晰度、季号和 TMDB/豆瓣确认；有限确认优先用于显式媒体 ID、作品页及正式名/别名明确命中的高可信候选。确认后的实际处理顺序为 TG 115、观影 115、PanSou 115、观影磁力、PanSou 磁力、聚影。普通手动搜索无需订阅，正式操作时才选择订阅并由后端重新确认；任一手动来源失败不会清空其它来源结果。PanSou 的 HTTP、业务、非 JSON、超时和网络错误不会写入空缓存。
