...
当前版本：`v4.7.32`。115 分享候选会受限地只读读取文件名，用真实文件名执行字幕、清晰度、季号和 TMDB/豆瓣确认；有限探测优先用于显式媒体 ID、观影作品页及正式名/别名明确命中的高可信候选。MoviePilot 识别标题不再重复拼接片名、年份与消息正文，确认后的实际处理顺序仍为 TG 115、观影 115、观影磁力、聚影。
