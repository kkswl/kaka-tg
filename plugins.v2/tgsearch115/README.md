...
当前版本：`v4.7.37`。PanSou 手动搜索使用实际可用的 GET `/api/search`，解析 `data.merged_by_type` 与 `results[].links`，统一标记聚合来源并保留上游来源；手动请求不限制网盘类型。前端拆分搜索来源和结果来源筛选，结果最多 500 条写入 `TgSearch115:manual-search:v1` 会话缓存，刷新可恢复，下一次有效搜索开始前清空，错误状态也会保存，缓存不含订阅、凭证或正式确认状态。普通手动搜索无需订阅，正式操作时才选择订阅并由后端严格确认；任一手动来源失败不会清空其它来源结果。PanSou 的 HTTP、业务、非 JSON、超时和网络错误不会写入自动搜索空缓存。
