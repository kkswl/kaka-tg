...
当前版本：`v4.7.38`。自动成功通知把来源数量明确标记为“搜索命中”，并从实际执行成功的候选显示“最终来源”；PanSou 的 `upstream_source` 会贯穿候选转换、MoviePilot 规则、媒体身份确认和通知。自动评估按来源记录原始返回、目标季初筛拒绝、去重拒绝和进入统一搜索池数量，MoviePilot 规则通过数继续独立诊断。PanSou 手动搜索使用 GET `/api/search` 并兼容真实聚合结构；手动结果使用最多 500 条的会话缓存。普通手动搜索无需订阅，正式操作时才选择订阅并由后端严格确认。
