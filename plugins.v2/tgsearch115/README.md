...
当前版本：`v4.7.19`。115 分享候选会受限地只读读取文件名，用真实文件名执行字幕、清晰度、季号和 TMDB/豆瓣确认；最多探测 3 个且缓存，不调用 `share_receive`。电视剧按目标季、TV 类型与 TMDB/豆瓣 ID 确认，订阅总年份与季首播年不同仅作诊断；电影年份仍严格校验。
...
需要实际处理已有订阅时，可调用受鉴权保护的 `POST /subscription/process` 并明确传入 `confirm=true`。它仅排入同一有界高优先级队列；只有全部身份与资源规则通过时才会触发真实转存或下载。
