...
class TgSearch115(_PluginBase):
    ...
    def init_plugin(self, config: dict = None):
        ...
        for _key, _old_vals in _legacy_old_defaults.items():
            if config.get(_key) in _old_vals:
            ...
            try:
                ...
                if status in (403, 429):
                    ...
                    if source_report:
                        source_report.mark(source, "error")
                    logger.warn(
                        f"【TG115】{source} 搜索失败分类={source_error}，本轮未缓存空结果（不限流，不触发冷却）"
                    )
                ...
                if source_report:
                    source_report.mark(source, "error")
                logger.warn(
                    f"【TG115】{source} 搜索异常，本轮跳过（不限流，不触发冷却）: {exc}"
                )
        return hits
